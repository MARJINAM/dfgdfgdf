...
client.elevation = message => {
  if(!message.guild) {
    return; }
  let permlvl = 0;
  if (message.member.hasPermission("")) permlvl = 1;
  if (message.member.hasPermission("")) permlvl = 2;
  if (message.member.hasPermission("")) permlvl = 3;
...
client.on('userUpdate', async (oldUser, newUser) => {
  ...
  if (newUser.username.includes(ayar.tag1)) {
    yuye.addRole(`613482212397940744`)
    client.channels.get(`613788628509917192`).send(`<@${oldUser.id}> **ismine **${ayar.tag1}** tagını eklediği için** <@&613482212397940744>  **rolü verildi! :gem:**`) // <<<< Burayı düzenle
  } else {
    yuye.removeRole(`613482212397940744`)
  }
  }
})
